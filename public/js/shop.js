$(document).ready(function () {
    ...
    function updateCartItemCount() {
        ...
        $('#cartItemCount').text(cartItemsLength);
        var totalPrice = 0;
        ...
        // Display total price in the cartTotal element
        $('#cartTotal').text('Total Price: ₱' + totalPrice.toFixed(2));
    }
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            $.each(data, function (key, value) {
                ...
                var itemHtml = "<div class='item mb-5' data-id='" + value.product_id + "'>" +
            ...
            $("#items").on('click', '.add-to-cart', function () {
                ...
                var selectedItem = data.find(item => item.product_id === itemId);
            ...
            $("#cartItems").on('click', '.remove-cart', function () {
    ...
    function addToCart(item) {
        ...
        var existingCartItem = cartItems.find(cartItem => cartItem.product_id === item.product_id);
    ...
    function displayCartItems(cartItems) {
        ...
        if (cartItems.length === 0) {
            ...
            cartItemsContainer.append(emptySvg);
            return;
        }

        $.each(cartItems, function (key, item) {
            var cartItemHtml = "<div class='cart-item row mb-3' data-id='" + item.product_id + "'>" +
                "<div class='col-md-3'>" +
                "<img src='/storage/" + item.image + "' class='w-200 h-200' alt='" + item.name + "' />" +
                "</div>" +
                "<div class='col-md-6'>" +
                "</div>" +
                ...
                "</div>" +
                "<button type='button' class='btn btn-danger mt-2 w-100 remove-cart'>Remove Item</button>" +
                "</div>" +
                "</div>";
    ...
    $("#checkout").on('click', function () {
        ...
        $("#cartItems .cart-item").each(function () {
            ...
            var cartItem = cartItems.find(item => item.product_id === itemId);
            if (cartItem) {
                cartItem.quantity = quantity;
                updatedQuantities.push({ product_id: itemId, quantity: quantity });
            }
        });
        var cartData = {
            cartItems: cartItems,
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                $("#invoiceNo").text(`Invoice No: ${data.orderlineId}`);
                $("#date").text(data.date);
                var totalAmount = 0;
                $.each(data.cartItems, function(index, item) {
                    var row = $('<tr>');
                    row.append($('<td>').text(item.name));
                    row.append($('<td class="text-center">').text('₱' + item.price));
                    row.append($('<td class="text-center">').text(item.quantity));
                    row.append($('<td class="text-center">').text('₱' + (parseFloat(item.price) * item.quantity).toFixed(2)));
                    var subtotal = parseFloat(item.price) * item.quantity;
                    totalAmount += subtotal;
                    $('#receipItemsTableBody').append(row);
                });
