...
<div class="center-container">
    <div class="transparent-box">
        ...
        <div class="container"
            ...
            <h2 style="margin-top: 150px;" >EDIT PRODUCT</h2>
            ...
            <form action="{{ route('updateProduct', ['id' => $product->product_id]) }}" method="post"
                ...
                <label for="category_id">CATEGORY:
                    <select name="category_id" id="category_id">
                        @foreach($category as $row)
                            <option value="{{ $row->category_id }}" @if($row->category_id == $product->category_id) selected @endif>
                                {{ $row->name }}
                            </option>
                        @endforeach
                        @if($category->isEmpty())
                ...
                <label for="name">PRODUCT NAME: <input type="text" id="name" name="name" value="{{$product->name}}" required></label>
                <label for="description">DESCRIPTION: <input type="text" id="description" name="description"
                    value="{{$product->description}}"   required></label>
                <label for="quantity">QUANTITY: <input type="number" id="quantity" name="quantity" value="{{$product->stock}}" required></label>
                <label for="price">PRICE: <input type="number" id="price" name="price" value="{{$product->price}}" required></label>
                ...
                <label for="seller_id">Seller:
                    <select name="seller_id" id="seller_id">
                        @foreach($seller as $row)
                            <option value="{{ $row->seller_id }}" @if($row->seller_id == $product->seller_id) selected @endif>
                                {{ $row->name }}
                            </option>
                        @endforeach
                        @if($seller->isEmpty())
