...
<div class="center-container">
    <div class="transparent-box">
        ...
        <div class="container"
            ...
            <h2>EDIT URN</h2>
            <form action="{{ route('updateUrn', ['id' => $urn->urn_id]) }}" method="post"
                enctype="multipart/form-data">
                @csrf
                <input type="hidden" name="niche_id">

                <label for="niche_id">Niche:
                    <select name="niche_id" id="niche_id">
                        @forelse ($result as $row)
                        <option value="{{ $row->id_niche }}" {{ $row->id_niche == $urn->niche_id ? 'selected' : '' }}>
                            {{ $row->building_name }} - Level-{{ $row->level }} - {{ $row->niche_number }}
                        </option>
                        @empty
                ...
                <label>URN NUMBER: <input type="number" id="urn_number" name="urn_number"
                        value="{{$urn->urn_number}}"></label>

                <!-- Uncomment the lines below if needed -->
                <!-- <label>DECEASED NAME: <input type="text" id="name" name="name" required></label> -->
                <!-- <label for="image">CHOOSE AN IMAGE: 
                    <input type="file" name="image" id="image" accept="image/*" required>
                </label> -->
                <!-- <label>Message <input type="text" id="message" name="message" required></label> -->
